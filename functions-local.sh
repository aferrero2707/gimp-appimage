...
delete_blacklisted()
{
  BLACKLISTED_FILES=$(cat $APPIMAGEBASE/excludelist | sed 's|#.*||g')
...
get_desktopintegration()
{
  ...
  cat $APPIMAGEBASE/AppImageKit/desktopintegration > ./usr/bin/$REALBIN.wrapper
...
generate_appimage()
{
  ...
  if [ ! -e $APPIMAGEBASE/AppImageAssistant ]; then
      wget -c "https://github.com/probonopd/AppImageKit/releases/download/6/AppImageAssistant_6-x86_64.AppImage" -O  $APPIMAGEBASE/AppImageAssistant # (64-bit)
      chmod a+x $APPIMAGEBASE/AppImageAssistant
  fi
  mkdir -p ../out || true
  rm ../out/$APP"-"$VERSION"-x86_64.AppImage" 2>/dev/null || true
  GLIBC_NEEDED=${GLIBC_NEEDED:=$(glibc_needed)}
  $APPIMAGEBASE/AppImageAssistant ./$APP.AppDir/ ../out/$APP"-"$VERSION".glibc"$GLIBC_NEEDED"-"$ARCH".AppImage"
}
